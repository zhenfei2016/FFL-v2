...
namespace android {
	...
	// 写数据到bytearray中
	//
	void JNIwriteByteArray(JNIEnv& env, jbyteArray byteArray,jsize capacity, uint8_t* data,uint32_t dataLen){
		if(dataLen<=capacity) {
			env.SetByteArrayRegion(byteArray, 0, dataLen, (jbyte *) data);
		}else{
			FFL_LOG_WARNING("JNIwriteByteArray len=%d,capcaity=%d",dataLen,capacity);
		}
	}
}
